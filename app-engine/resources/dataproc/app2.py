...
from gcp import create_cluster, delete_cluster, show_cluster_status
...
job_model = api.model('Job', {
    'id': fields.String(readonly=False, description='The job unique identifier'),
...
class ClusterOperator(object):
    """Class representing a Dataproc operator and its configuration."""
    
    def __init__(self):
        self.clusters = []

    def get(self, cluster_name:str) -> Optional[dict]:
        """Get an existing dataproc cluster instance
        
        Args:
            cluster_name(str): The name of cluster
        Returns:
            cluster(dict): The dataproc cluster
        """
    ...
    def create(self, input_data) -> Optional[dict]:
        """Create a dataproc cluster instance
        Args:
            input_data(dict): The input data of Swagger UI
        Returns:
            cluster(dict): The dataproc cluster
        """
...
class ClusterList(Resource):
    ...
    def post(self):
        ...
        status = show_cluster_status(project_id=gcp_project_id, region=region, cluster_name=cluster_name)
...
class Cluster(Resource):
    ...
    def get(self, cluster_name):
        '''Get a cluster'''
        status = show_cluster_status(project_id=gcp_project_id, region=region, cluster_name=cluster_name)
        cluster = ClusterOperator.get(cluster_name)
